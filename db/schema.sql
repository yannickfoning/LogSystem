...
CREATE TABLE `logs` (
  ...
  `ingested_realtime` TINYINT(1) DEFAULT 0,
  ...
  `log_source` VARCHAR(255) DEFAULT NULL,
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  ...
  INDEX `idx_imported_by_user_id` (`imported_by_user_id`),
...
CREATE TABLE `import_jobs` (
  ...
  `status` ENUM('pending','depositing','extracting','parsing','counting','inserting','processing','completed','failed') DEFAULT 'pending',
  `total_lines` INT DEFAULT 0,
  `processed_lines` INT DEFAULT 0,
  `successful_lines` INT DEFAULT 0,
  ...
  `import_summary` JSON DEFAULT NULL,
  `started_at` DATETIME,
  `completed_at` DATETIME,
  `created_at` DATETIME DEFAULT CURRENT_TIMESTAMP,
  `updated_at` DATETIME DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  `total_files` INT DEFAULT 1,
  `depot_path` VARCHAR(512) DEFAULT NULL,
  `treated_path` VARCHAR(512) DEFAULT NULL,
