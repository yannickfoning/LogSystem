...
import { runRetention, getRetentionStats } from '../services/retentionService.js';
import { validateBody, alertRuleSchema, alertUpdateSchema, createUserSchema, updateUserSchema, resetPasswordSchema, purgeSchema } from '../middleware/validation.js';
...
router.put('/users/:id', validateBody(updateUserSchema), async (req, res) => {
  try {
    ...
    if (is_active === false && req.session.user.id === userId) {
      return res.status(403).json({ error: 'Vous ne pouvez pas vous désactiver' });
    }
...
router.post('/alert-rules', validateBody(alertRuleSchema), async (req, res) => {
  try {
    const { name, description, condition_type, condition_value, threshold_value, time_window_minutes, severity, cooldown_minutes } = req.body;
    ...
    const [result] = await pool.execute(
      ...
      [name, description || null, condition_type, condition_value, threshold_value || null, time_window_minutes || 60, severity || 'medium', cooldown_minutes || 30, req.session.user.id]
    );
...
router.put('/alert-rules/:id', async (req, res) => {
  try {
    const { name, description, condition_type, condition_value, threshold_value, time_window_minutes, severity, cooldown_minutes, is_active } = req.body;

    const fields = [];
    const params = [];
    if (name !== undefined) { fields.push('name = ?'); params.push(name); }
    if (description !== undefined) { fields.push('description = ?'); params.push(description); }
    if (condition_type !== undefined) { fields.push('condition_type = ?'); params.push(condition_type); }
    if (condition_value !== undefined) { fields.push('condition_value = ?'); params.push(condition_value); }
    if (threshold_value !== undefined) { fields.push('threshold_value = ?'); params.push(threshold_value); }
    if (time_window_minutes !== undefined) { fields.push('time_window_minutes = ?'); params.push(time_window_minutes); }
    ...
    const [result] = await pool.execute(`UPDATE alert_rules SET ${fields.join(', ')} WHERE id = ? AND created_by = ?`, params);
    if (result.affectedRows === 0) return res.status(404).json({ error: 'RÃ¨gle non trouvÃ©e' });
...
router.delete('/alert-rules/:id', async (req, res) => {
  try {
    const [owned] = await pool.execute('SELECT id FROM alert_rules WHERE id = ? AND created_by = ?', [req.params.id, req.session.user.id]);
    if (owned.length === 0) return res.status(404).json({ error: 'RÃ¨gle non trouvÃ©e' });
    await pool.execute('DELETE FROM alerts WHERE rule_id = ? AND user_id = ?', [req.params.id, req.session.user.id]);
    const [result] = await pool.execute('DELETE FROM alert_rules WHERE id = ? AND created_by = ?', [req.params.id, req.session.user.id]);
    if (result.affectedRows === 0) return res.status(404).json({ error: 'Règle non trouvée' });
...
router.patch('/alerts/:id', async (req, res) => {
  try {
    ...
    if (!['dismissed', 'read', 'new'].includes(status)) {
      return res.status(400).json({ error: 'Status invalide. Accepté: new, read, dismissed' });
    }
    ...
    // Only allow user to modify their own alerts, or admin to modify any
    if (alert.user_id !== req.session.user.id) {
    ...
    await pool.execute(
      'UPDATE alerts SET status = ?, read_at = ? WHERE id = ? AND user_id = ?',
      [status, readAt, alertId, req.session.user.id]
    );
    ...
    logger.error({ event: 'alert_status_error', error: e.message }, '[ALERT STATUS]');
    res.status(500).json({ error: 'Erreur serveur' });
  }
});
...
router.get('/audit', async (req, res) => {
  try {
    ...
    if (user_id && Number(user_id) !== req.session.user.id) return res.status(403).json({ error: 'AccÃ¨s refusÃ©' });
    if (action) { sql += ' AND action = ?'; params.push(action); countSql += ' AND action = ?'; countParams.push(action); }
    if (resource_type) { sql += ' AND resource_type = ?'; params.push(resource_type); countSql += ' AND resource_type = ?'; countParams.push(resource_type); }
    if (date_from) { sql += ' AND created_at >= ?'; params.push(date_from); countSql += ' AND created_at >= ?'; countParams.push(date_from); }
    if (date_to) { sql += ' AND created_at <= ?'; params.push(date_to); countSql += ' AND created_at <= ?'; countParams.push(date_to); }
    ...
    res.json({
      data: rows,
      pagination: { page: pageVal, limit: limitVal, total, pages: Math.ceil(total / limitVal) }
    });
  } catch (e) {
    res.status(500).json({ error: 'Erreur serveur' });
  }
...
router.post('/purge', validateBody(purgeSchema), async (req, res) => {
  try {
    ...
    if (date_before) { sql += ' AND timestamp < ?'; params.push(date_before); }
...
router.get('/system-stats', async (req, res) => {
  try {
    ...
    const [errorGroups] = await pool.execute('SELECT COUNT(*) as cnt FROM error_groups WHERE status = "open" AND user_id = ?', [user.id]);
    const [alerts]      = await pool.execute(
      `SELECT COUNT(*) as cnt FROM alerts WHERE status = 'new'${scopeSql}`, scopeParams
    );
