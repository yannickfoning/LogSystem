...
router.get("/audit", async (req, res) => {
  try {
    ...
    // Join users to get display_name; use COALESCE in case status column was not yet migrated
    sql = sql.replace(
      'SELECT * FROM audit_log WHERE 1=1',
      'SELECT al.*, u.display_name FROM audit_log al LEFT JOIN users u ON u.id = al.user_id WHERE 1=1'
    );
    sql += ` ORDER BY al.created_at DESC LIMIT ${limitVal} OFFSET ${offset}`;

    let rows;
    try {
      [rows] = await pool.query(sql, params);
    } catch (e2) {
      // Fallback without JOIN if audit_log structure differs
      const simpleSql = countSql.replace('SELECT COUNT(*) as total FROM audit_log WHERE 1=1', 'SELECT * FROM audit_log WHERE 1=1')
        + ` ORDER BY created_at DESC LIMIT ${limitVal} OFFSET ${offset}`;
      [rows] = await pool.query(simpleSql, params);
    }
