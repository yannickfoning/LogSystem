...
export interface DashboardSystem {
  system: any; uptime?: number; memory?: Record<string, number>; cpu?: Record<string, number>; disk?: Record<string, number> 
}
...
export interface LogQueryParams { page?: number; limit?: number; level?: string; logLevel?: string; source?: string; service?: string; application?: string; directory?: string; search?: string; keyword?: string; dateFrom?: string; dateTo?: string; startDate?: string; endDate?: string; timeFrom?: string; timeTo?: string; }
export interface LogListResponse { logs: LogEntry[]; pagination: { page: number; limit: number; total: number; totalPages: number } }
export interface AlertItem { id: string; alertType?: string; severity: string; message?: string; status: string; occurrenceCount?: number; firstOccurrence?: string; lastOccurrence: string; errorType?: string; sourceDirectory?: string; affectedUser?: string; createdAt: string; ruleId?: string; }
export interface AlertRule { id: string; name: string; description?: string; conditionType: string; conditionValue: string; thresholdValue?: number | null; timeWindowMinutes: number; severity: string; cooldownMinutes: number; isActive: boolean; enabled?: boolean; }
...
export interface CreateAlertRulePayload { name: string; description?: string; conditionType: string; conditionValue: string; thresholdValue?: number | null; timeWindowMinutes: number; severity: string; cooldownMinutes: number; isActive?: boolean; enabled?: boolean; }
export interface ImportJob {
  originalName: ReactNode; id: string; filename?: string; status: string; totalLines: number; processedLines: number; errorCount: number; skippedLines?: number; importSummary?: string; createdAt: string; completedAt?: string; 
}
export interface ImportJobsResponse { jobs: ImportJob[] }
export interface AuditLogEntry { id: string; userEmail?: string; action: string; resourceType?: string; resourceId?: string; details?: string; ipAddress?: string; status: string; createdAt: string; }
export interface AuditLogQueryParams { page?: number; limit?: number; action?: string; }
export interface AuditLogResponse { auditLogs: AuditLogEntry[]; pagination: { page: number; limit: number; total: number; totalPages: number } }
...
export interface PurgePayload { level?: string; startDate?: string; endDate?: string; olderThanDays?: number; }
