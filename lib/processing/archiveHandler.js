...
export const LOG_FILE_PATTERN = /\.(log|txt|json|jsonl|csv)$/i;
...
/**
 * Keep only log-relevant files from an archive listing
...
/**
 * Map extraction errors to user-facing French messages
 * @param {Error} err
 */
export function mapArchiveError(err) {
  const msg = String(err?.message || '');
  if (err?.code === 'RAR_ENCRYPTED' || /password|encrypted|mot de passe/i.test(msg)) {
    return new ArchiveError('RAR_ENCRYPTED', 403, 'Archive protégée par mot de passe — non supportée. Utilisez RAR sans mot de passe.');
  }
  if (/corrupt|invalid|Unexpected end|failed to open/i.test(msg)) {
    return new ArchiveError('RAR_CORRUPT', 400, 'Archive RAR invalide ou corrompue. Téléchargez à nouveau.');
  }
  if (err?.code === 'NO_LOG_FILES') {
    return err;
  }
  if (/too large|exceeds maximum/i.test(msg)) {
    return new ArchiveError('FILE_TOO_LARGE', 413, 'Fichier trop gros (500 MB max). Divisez en plusieurs archives.');
  }
  return new ArchiveError('ARCHIVE_ERROR', 400, msg || 'Erreur lors de l\'extraction de l\'archive.');
}
    ...
    unzipStream.on('entry', async (entry) => {
      ...
      entry.on('end', () => {
        files.push({
          ...
          file_modified_at: normalizeArchiveDate(entry.vars?.lastModifiedDate || entry.props?.lastModifiedDate)
        });
        pendingEntries--;
 ...
 */
async function extractBrotli(_buffer, _maxSize) {
 ...
 */
async function extractZstandard(_buffer, _maxSize) {
 ...
 * Extract TAR archive
 */
async function extractTar(_buffer, _maxSize) {
  try {
    const files = await decompress(Buffer.from(_buffer), '.');
    ...
    for (const file of files) {
      ...
      results.push({
        filename: file.path,
        content: file.data,
        file_modified_at: normalizeArchiveDate(file.mtime || file.stat?.mtime)
      });
    }
    
    return results;
  } catch (e) {
    throw new Error(`TAR extraction failed: ${e.message}`, { cause: e });
  }
}
  ...
  try {
    ...
    for (const h of fileHeaders) {
      if (h.flags?.encrypted) {
        throw new ArchiveError('RAR_ENCRYPTED', 403, 'Archive protégée par mot de passe — non supportée');
      }
    }
    ...
    for (const file of extracted.files) {
      ...
      const modifiedAt = normalizeArchiveDate(
        file.fileHeader.mtime ||
        file.fileHeader.time ||
        file.fileHeader.fileTime ||
        file.fileHeader.lastModifiedTime
      );
      ...
      if (!SKIPPED_EXTENSIONS.has(ext)) {
        files.push({
          ...
          file_created_at: normalizeArchiveDate(file.fileHeader.ctime || file.fileHeader.creationTime)
        });
      }
    }
    ...
    if (jsErr instanceof ArchiveError) throw jsErr;
    if (/password|encrypted/i.test(jsErr.message)) {
      throw new ArchiveError('RAR_ENCRYPTED', 403, 'Archive protégée par mot de passe — non supportée');
    }
    ...
    try {
      ...
      for (const filePath of extractedFiles) {
        ...
        totalSize += stat.size;
        if (totalSize > maxSize) {
          throw new Error(`Archive too large (> ${maxSize} bytes)`, { cause: jsErr });
        }
        ...
        files.push({
          ...
          content: fileContent,
          file_created_at: normalizeArchiveDate(stat.birthtime),
          file_modified_at: normalizeArchiveDate(stat.mtime)
        });
      }

      return files;
    } catch (binErr) {
      // Both extraction layers failed — surface the original (often more useful) JS error
      // alongside the binary fallback error, instead of hiding the root cause.
      throw new Error(
        `RAR extraction failed. node-unrar-js: ${jsErr.message} | system binary: ${binErr.message}`,
        { cause: binErr }
      );
    } finally {
...
async function extractSevenZip(buffer, filename, maxSize) {
  ...
  try {
    ...
    throw new Error(`7z extraction failed: ${e.message}`, { cause: e });
  } finally {
    await fs.rm(tempRoot, { recursive: true, force: true });
  }
}
