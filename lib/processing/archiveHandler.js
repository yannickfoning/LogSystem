...
// Load WASM binary manually — Vercel doesn't auto-bundle .wasm files referenced
// via dynamic paths inside node_modules. We read it at startup and pass it as
// wasmBinary so node-unrar-js never tries to fetch it from disk at runtime.
...
async function getUnrarExtractor() {
  ...
  try {
    ...
    for (const p of wasmPaths) {
      try { wasmBinary = await fs.readFile(p); break; } catch (_) {}
    }
...
async function extractRar(buffer, filename, maxSize) {
  ...
  try {
    if (!createExtractorFromData) throw new Error('node-unrar-js not available');
  ...
  try {
    ...
    if (e instanceof ArchiveError) throw e;
    throw new ArchiveError('RAR_EXTRACTION_FAILED', 400, `Extraction RAR échouée: ${e.message}`);
  }
}
