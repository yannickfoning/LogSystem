...
const EXTRACT_TIMEOUT_MS = IS_VERCEL ? 120000 : 30000;
...
async function extractRar(buffer, filename, maxSize) {
  logger.info({ event: 'rar_extract_start', filename, isVercel: IS_VERCEL }, '[ARCHIVE]');
  ...
  // 7za does not support RAR on all builds — last-resort attempt for simple archives
  try {
