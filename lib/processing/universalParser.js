...
import { detectJavaLog4jFormat, parseJavaLog4jBatch } from './javaLog4jParser.js';
import { detectJavaLog4jFormat as detectJavaLog4jStreamFormat, parseJavaLog4jStream } from './javaLog4jStreamParser.js';
...
const PATTERNS = {
  ...
  javaLog4j: /^(\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2},\d{3})\s+(TRACE|DEBUG|INFO|WARN|ERROR|FATAL)\s+\[([^\]]+)\]\s+(.*)$/,
...
export async function parseLogContent(buffer, detectedFormat = null, opts = {}) {
  ...
  try {
    // Special handling for large Java Log4j files with multi-line support
    // Use streaming parser for files > 10MB to avoid memory issues
    const LARGE_FILE_THRESHOLD = 10 * 1024 * 1024; // 10MB
    const isLargeFile = buffer.length > LARGE_FILE_THRESHOLD;
    const isJavaLog4j = format === 'java' || detectJavaLog4jFormat(buffer) || detectJavaLog4jStreamFormat(buffer);

    if (isJavaLog4j) {
      logger.info({ event: 'using_java_log4j_parser', format, isLargeFile, fileSize: buffer.length }, '[PARSER]');

      if (isLargeFile && opts.onBatch) {
        // Use streaming parser for large files with batch callback
        const result = await parseJavaLog4jStream(buffer, {
          ...opts,
          onBatch: async (batch) => {
            // Normalize each log in the batch
            const normalizedBatch = batch.map(log => normalizeLog(log, 'java', opts));
            await opts.onBatch(normalizedBatch);
          }
        });
        return []; // Logs are processed via onBatch callback
      } else {
        // Use batch parser for smaller files or when no callback provided
        const result = parseJavaLog4jBatch(buffer, opts);

        // Convert batches to normalized logs
        const normalizedLogs = [];
        for (const batch of result.batches) {
          for (const log of batch) {
            normalizedLogs.push(normalizeLog(log, 'java', opts));
          }
        }
...
function parseJavaLogs(text, opts = {}) {
  ...
  for (const line of lines) {
    ...
    if (m) {
      ...
      const level = m[2] === 'WARN' ? 'WARNING' : m[2].toUpperCase();
      ...
      // Multi-line continuation (stack traces)
      const isStackTrace = /^\s+at\s+|^Caused by:/.test(line);
      if (isStackTrace) {
  ...
  return logs.map(log => normalizeLog(log, 'java', opts));
}
