...
function extractTimestampFromText(text) {
  ...
  const patterns = [
    ...
    /\b\d{4}-\d{2}-\d{2}\s+\d{2}:\d{2}:\d{2}(?:,\d+|\.\d+)?\b/,
...
function normalizeLog(log, format, opts = {}) {
  ...
  // Normalize timestamp to ISO 8601 complete format (YYYY-MM-DDTHH:mm:ss.fffZ or +/-HH:mm)
  ...
  // AMÉLIORATION 1: Extract target_user (distinct from log owner)
  // Pattern alternatives: user=, actor=, by=, username=, account=, uid=, client=
  ...
  let target_user = log.target_user || log.user || log.actor || log.by ||
                    ...
                    extractNamedValue(rawMessage, ['user', 'username', 'uid', 'actor', 'account', 'email']) || null;
...
/**
 * Normalize timestamp to ISO 8601 format (YYYY-MM-DDTHH:mm:ss.fffZ)
 */
function normalizeTimestamp(ts) {
  ...
  try {
    ...
    // Common format: "2025-05-24 14:32:11" or "2025-05-24T14:32:11"
    else if (str.match(/^\d{4}-\d{2}-\d{2}\s\d{2}:\d{2}:\d{2}/)) {
      date = new Date(str.replace(' ', 'T'));
    }
    // Syslog format: "May 24 14:32:11"
    else if (str.match(/^\w{3}\s+\d{1,2}\s+\d{2}:\d{2}:\d{2}/)) {
      const year = new Date().getFullYear();
