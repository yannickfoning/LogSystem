...
// Test CSRF protection by making a request without CSRF token
const loginOptions = {
  hostname: '127.0.0.1',
  port: 3001,
  path: '/api/auth/login',
...
const loginReq = http.request(loginOptions, (loginRes) => {
  ...
  if (sessionCookie) {
    ...
    console.log('Testing CSRF protection (request without CSRF token)...');
    ...
    const csrfTestReq = http.request(csrfTestOptions, (csrfTestRes) => {
      ...
      csrfTestRes.on('end', () => {
        if (csrfTestRes.statusCode === 403) {
          ...
          console.log('CSRF Response:', data);
          console.log('\n⚠️ CSRF protection may not be properly configured');
        }
      });
    });
